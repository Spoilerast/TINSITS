...
{
	internal PrismProperties(byte tid,
							 PrismType type,
							 float integrity,
							 float capacity,
							 float charge,
							 float resistance)
	{
		(Team, Type, Integrity, Capacity, Charge, Resistance) =
			(TeamId.GetTeamId(tid), type, integrity, capacity, charge, resistance);
	}
	...
	public override string ToString()
		=> $"pp {Team},   tp{Type} i{Integrity} ca{Capacity} ch{Charge} r{Resistance}";
