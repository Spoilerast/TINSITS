...
internal sealed class PropertiesDB
{
	...
	//private float Capacity => _properties[_currentId].Capacity;

	private float Charge => _properties[_currentId].Charge;

	private float Integrity => _properties[_currentId].Integrity;

	private float Resistance => _properties[_currentId].Resistance;

	//private TeamId Team => _properties[_currentId].Team;

	private PrismType Type => _properties[_currentId].Type;

	//internal void Add(UnitId id) { _properties.Add(id, new()); }

	internal void Add(UnitId id, PrismProperties properties) { _properties.Add(id, properties); }
	...
	internal void DamageUnitAndNeighbors(UnitId victimId, in float attackPower, ref UnitId[] neighbors)
	{
		...
		partDamage.LogThis();
		foreach (UnitId neighbor in neighbors)           //todo maybe recursive damage chain for neighbors of neighbors
	...
	{
		(TeamId tid, PrismType type, float i, float ca, float cha, float re) = _properties[id];
	...
	internal PrismType SetPrismType(UnitId id, PrismType prismType) { return _properties[id].Type = prismType; }
	...
	private void ClearCurrentCharge() { ClearCharge(_currentId); }
	...
	private void SetCharge(UnitId id, in float value) { _properties[id].Charge = value; }

	private void SetCurrentCharge(in float v) { SetCharge(_currentId, v); }

	private void SetIntegrity(in float value) { _properties[_currentId].Integrity = value; }
}
}