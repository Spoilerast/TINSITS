...
internal sealed partial class ConnectionsDB
{
	...
	internal void Debug_PrintNets(string mes = null) //todo remove
	{
		...
		foreach (KeyValuePair<ushort, Net> net in _nets){
	...
	{
		PeekLogger.LogName();
		if (_connectedUnits.TryGetValue(disconnected[0], out ushort netId)){
	...
	{
		GetNetAndUnitsPoints(clusterUnitIds, out ushort netId, out IEnumerable<(UnitId, GridPoint)> unitsPoints);
	...
	private void ClusterConnections(UnitId unitId)
	{
		...
		foreach ((UnitId idA, GridPoint positionA, UnitId idB, GridPoint positionB) in allConnections){
			PeekLogger.LogItemsVarious(idA, idB);
			MakeConnection(idA, positionA, idB, positionB);
		}
	}

	private void ConnectOneWithMany(UnitId unitId,
									GridPoint unitPosition,
									ref IEnumerable<(UnitId, GridPoint)> connections)
	{
		foreach ((UnitId neighborId, GridPoint neighborPosition) in connections)
	...
	private bool ContainsLink(GridPoint pointA, GridPoint pointB)
		=> _links.Exists(x => x.IsConnectionEqual(pointA, pointB));

	private void CreateLink(UnitId idA,
							GridPoint positionA,
							UnitId idB,
							GridPoint positionB,
							ref ushort netId)
	...
	private void GetNetAndUnitsPoints(IEnumerable<UnitId> clusterUnitIds,
									  out ushort netId,
	...
	private void MakeConnection(UnitId initiatorId,
	...
	{
		foreach (ushort netId in _netsToIntegralityCheck)
	...
	private void SingleConnections(UnitId unitId)
	{
		if (ConnectionProcessor
			.GetSingleConnections(unitId, out GridPoint unitPosition,
								  out IEnumerable<(UnitId, GridPoint)> reconnections)
		   )
	...
	private void ToReconnection(UnitId unitId) { _unitsToReconnection.Push(unitId); }
