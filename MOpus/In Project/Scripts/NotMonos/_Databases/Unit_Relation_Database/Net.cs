...
internal sealed partial class ConnectionsDB
{
	private sealed class Net
	{
		...
		private Net(Dictionary<UnitId, NetNode> nodes, SortedDictionary<ushort, Link> links) { (_nodes, _links) = (nodes, links); }
		...
		internal int CountUnits => _nodes.Count;
		...
		internal IEnumerable<UnitId> UnitsIds => _nodes.Keys;
		...
		internal void AddLink(Link link)
		{
			...
			(UnitId a, UnitId b) = link.GetUnits();
			_nodes[a].Add(linkId);
			_nodes[b].Add(linkId);
		}
		...
		{
			foreach (UnitId item in unitIds){
		...
		{
			neighbors = (
					from linkId in _nodes[unitId].Links
		...
		internal void MergeWith(Net net)
		{
			...
			foreach (Link link in net._links.Values)
				AddLink(link);
		}

		internal void RemoveUnit(UnitId unitId)
		{
			if (!_nodes.TryGetValue(unitId, out NetNode netNode))
				return;

			IEnumerable<ushort> unitLinks = netNode.Links;
			IEnumerable<(UnitId, ushort linkId)> nodesForRemoveLinks =
				from linkId in unitLinks
				select (_links[linkId].GetNeighborOf(unitId), linkId);
			//PeekLogger.LogItems(nodesForRemoveLinks);

			foreach ((UnitId node, ushort link) in nodesForRemoveLinks){
		...
		internal void RemoveUnits(UnitId[] disconnected)
		{
			IEnumerable<ushort> disconnectedLinks =
			(
				from uid in disconnected
				from lid in _nodes[uid].Links
				select lid).Distinct();
			...
			(UnitId NeighborId, ushort linkId)[] neighborsOfDisconnected =
			(
				from linkId in disconnectedLinks
				from uid in disconnected
				let check = new {
					HaveNeighbor = _links[linkId].TryGetNeighborOf(uid, out UnitId neighborId),
					NeighborId = neighborId
				}
				where check.HaveNeighbor && !disconnected.Contains(check.NeighborId)
			...
			foreach ((UnitId id, ushort link) in neighborsOfDisconnected){
				_ = onReconnection.Add(id);
				_nodes[id]
					.RemoveLink(link); //preventing wild Ids in this Node (link is deleted, but Node still have it)
			}

			foreach (ushort item in disconnectedLinks)
				RemoveLink(item);

			foreach (UnitId item in disconnected)
				RemoveNode(item);
		...
		private void RemoveNode(UnitId unitId) { _nodes.Remove(unitId); }
								  ...
								  out Net net)
		{
			...
			foreach (UnitId item in unitsIds){
				nodes.Add(item, _nodes[item]);
				_ = _nodes.Remove(item);
			}

			foreach (ushort item in linksIds){
