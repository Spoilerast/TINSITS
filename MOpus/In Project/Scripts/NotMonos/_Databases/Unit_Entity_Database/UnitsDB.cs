...
internal sealed class UnitsDB
{
	...
	internal IEnumerable<UnitId> AllUnitIds => _units.Keys;
	...
	internal void Clear() { _units.Clear(); }
	...
	{
		UnitId[] clusterUnits = GetUnitsInCluster(clusterId).ToArray();
		foreach (UnitId item in clusterUnits)
