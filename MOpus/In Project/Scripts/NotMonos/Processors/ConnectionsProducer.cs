...
{
internal abstract partial class ConnectionProcessor //todo: questionable place, reconsider
{
	private sealed class ConnectionsProducer //maybe abstract
	{
		...
		internal ConnectionsProducer(UnitsDB units, ClustersDB clusters) { (_units, _clusters) = (units, clusters); }
		...
		internal IEnumerable<(UnitId, GridPoint, UnitId, GridPoint)> MakeClusterConnectionsFor(UnitId unitId)
		{
			...
			//level 2
			Constants.VectorsOrientation clusterOrientation = GetClusterOrientation(out GridPoint start);
			start -= CS.ClusterRight;
			ClusterLevel2Connections(start, clusterOrientation);

			//level 3
			Constants.VectorsOrientation startOrientation = CS.GetVectorOrientation(start);
			GridPoint variable = clusterOrientation is Constants.VectorsOrientation.Bottom
				? start + CS.Forward
				: start + CS.BackRight;
			start = startOrientation is Constants.VectorsOrientation.Top
				? variable
				: start + _way;
		...
		internal IEnumerable<(UnitId, GridPoint, UnitId, GridPoint)> MakeSuperClusterConnectionsFor(UnitId unitId)
		{
			...
			//level 3
			Constants.VectorsOrientation orientation = CS.GetVectorOrientation(start);

			start = orientation is Constants.VectorsOrientation.Top
		...
		{
			Connectability ability = DirectionValidForLongConnection(_initiatorId, point, out UnitId neighborId);
		...
		{
			Connectability ability = DirectionValidForLongestConnection(_initiatorId, point, out UnitId neighborId);
			if (ability is Connectability.Unconnectable)
				return;
		...
		private void ClusterHexagonConnections(GridPoint start, LinkLevel level)
		{
			...
			GridPoint vector = vectors.Dequeue();
			start += vector;
			ChooseCheck();
		...
		private void ClusterLevel2Connections(GridPoint start, Constants.VectorsOrientation orientation)
		{
		...
		private void ClusterLevel3Connections(GridPoint start,
											  Constants.VectorsOrientation clusterOrientation,
											  Constants.VectorsOrientation startOrientation)
		{
			if (clusterOrientation is Constants.VectorsOrientation.Bottom){
				if (startOrientation is Constants.VectorsOrientation.Top)
					ClusterHexagonConnections(start, LinkLevel.Three);
				else
					ClusterPyramidConnections(start, clusterOrientation);
		...
		private void ClusterPyramidConnections(GridPoint start, Constants.VectorsOrientation orientation)
		{
			Queue<GridPoint> vectors = new(CS.ClusterPyramidVectors(orientation));
		...
		private bool DirectionValidForConnectionLevel1(GridPoint direction,
		...
		private Constants.VectorsOrientation GetClusterOrientation(out GridPoint start)
		{
			//not effective but short and readable
			ClusterInfo info = _initiatorCluster;
			//start = info.Positions.OrderBy(v=>v.X).First();
			start = info.Positions.MinBy(v => v.X);
			float minimumZ = info.Positions.Min(v => v.Z);
			GridPoint[] points = info.Positions.Where(v => GridPoint.AreFloatEquals(v.Z, minimumZ))
									 .ToArray();
			return points.Length == 1
				? Constants.VectorsOrientation.Bottom
				: Constants.VectorsOrientation.Top;
		...
		private void JigsawLoopLongestCheck(int oddIterationsCount,
											int evenIterationsCount,
											ref GridPoint start,
											ref Queue<GridPoint> vectors,
											ref GridPoint vector)
		{
		...
		private void Level1Connections()
		{
			...
			foreach (GridPoint vertex in _initiatorCluster.Positions){
				if (!Grid.TryGetUnitId(vertex, out idA))
					continue;

				foreach (GridPoint direction in CS.GetMoveDirectionPoints(vertex)){
					if (!DirectionValidForConnectionLevel1(direction, out idB, out ClusterId neighborClusterId))
						continue;

					if (!TryGetPointsToLevel1Connection(neighborClusterId, vertex, direction, out pointA, out pointB))
						continue;
		...
		private void SuperClusterLevel3Connections(GridPoint start, Constants.VectorsOrientation orientation)
		{
		...
		private bool TryGetPointsToLevel1Connection(ClusterId clusterId,
													GridPoint vertex,
													GridPoint direction,
													out GridPoint pointA,
													out GridPoint pointB)
		{
			pointA = null;
			pointB = null;
			...
			if (_alreadyLinkedClusters.Contains(clusterId))
				return false;

			pointA = _initiatorAxis;
			pointB = Clusters.GetClusterInfo(clusterId).AxisPoint;
			_alreadyLinkedClusters.Add(clusterId);
			return true;
		}
