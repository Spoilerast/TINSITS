...
{
internal abstract class AttackProcessor : Processor
{
	...
	{
		IEnumerable<UnitId> units = Units.GetClusterNeighborsIds(attackerId)
										 .Append(attackerId)
										 .ToArray();
	...
	{
		IEnumerable<UnitId> units = neighbors.Append(attackerId).ToArray();
		float power = Properties.GetAttackAmount(units);
	...
	private static void ReceiveAttackSingle(UnitId victimId, in float attackPower) { Properties.DamageUnit(victimId, attackPower, out _); }
}
}