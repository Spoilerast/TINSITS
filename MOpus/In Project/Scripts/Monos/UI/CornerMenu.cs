...
internal sealed class CornerMenu : SceneUI
{
	...
	public void Initialize(SceneObjectsSpawner spawner) { _spawner = spawner; }

	internal void ToggleMenu(CornerMenuState state) { FoldSetState(state); }

	private void ButtonClickedChangeTeam(EventBase evt)
	{
		TeamId changeTo = SceneGlobals.CurrentTeam.Equals(1)
			? TeamId.GetTeamId(2)
			: TeamId.GetTeamId(1);
	...
	private void CornerMouseEnter(EventBase evt) { FoldSetState(CornerMenuState.Opened); }

	private void CornerMouseLeave(EventBase evt) { FoldSetState(CornerMenuState.Closed); }
	...
	private void NavCancel(NavigationCancelEvent evt) { FoldSetState(CornerMenuState.Closed); }
}
}