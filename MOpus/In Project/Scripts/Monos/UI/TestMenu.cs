...
internal class TestMenu : SceneUI //todo remove it
{
	...
	private void ButtonClicked(EventBase evt) { _label.text = $"{((Button)evt.currentTarget).name} clicked by gamepad"; }

	private void ButtonClicked(ClickEvent evt) { _label.text = $"{((Button)evt.currentTarget).name} clicked by mouse"; }

	private void ButtonFocused(FocusEvent evt) { _label.text = ((Button)evt.currentTarget).name + " focused"; }
}
}