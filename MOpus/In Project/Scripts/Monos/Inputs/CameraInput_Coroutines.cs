...
namespace Inputs
{
[Obsolete("Async version is better. Code here is created for example to compare between two versions."
		  + "Not maintained. Contains some temporal code and comments. Very old.")][DisallowMultipleComponent]
internal sealed class CameraInput_Coroutines : SceneSystem
{
	...
	{
		Vector3 x, y;
		InputAction action = _inputs.Camera.MouseMovement;
	...
	private IEnumerator MovementRoutine()
	{
		...
		InputAction action = _inputs.Camera.Movement;
		while (action.inProgress){
	...
	private IEnumerator RotationXRoutine()
	{
		float rotationValue;
		Vector3 euler;
		InputAction action = _inputs.Camera.RotationX;
		while (action.inProgress){
	...
	{
		float rotationValue;
		InputAction action = _inputs.Camera.RotationY;
		while (action.inProgress){
	...
	{
		InputAction action = _inputs.Camera.Zoom;
		while (action.inProgress){
