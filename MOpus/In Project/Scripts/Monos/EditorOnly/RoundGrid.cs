...
{
public sealed class RoundGrid : EditorSystem //dis code is too old for all dis sh...
{
	...
	private void DrawGrid()
	{
		...
		if (_gridRadius.x != gridRadiusX || _gridRadius.z != gridRadiusZ || !Mathf.Approximately(_gridStep, gridStep))
			_posnts = GetGridPositions();

		GUI.color = textColor;
		Handles.color = new(.3f, .3f, .3f, 1);
		foreach (Vector3 i in _posnts){
			Vector3 ii = i + gridOffset;
			Handles.DrawWireDisc(ii, Vector3.up, .5f);
			if (turnOnGridCoords)
				Handles.Label(ii, $"{ii.x:f0}, {ii.z:f0}");
		}
	}
	...
	{
		yield return new(center.x + half, center.y, center.z + quarter);
		yield return new(center.x + half, center.y, center.z - quarter);
		yield return new(center.x - half, center.y, center.z + quarter);
		yield return new(center.x - half, center.y, center.z - quarter);
		yield return new(center.x, center.y, center.z + half + quarter);
		yield return new(center.x, center.y, center.z - (half + quarter));
	}
